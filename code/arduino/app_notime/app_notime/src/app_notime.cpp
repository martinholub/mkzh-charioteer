...
// init
int gps_avg_per = 6; // average over <n> measurements, report only then
...
#define send_per 60 //gps_per*gps_avg_per
...
void add_buffer_dyn(float speed, float dist, int it = 0) {
  ...
  velB = (round(speed * 100.0) / 100.0) / 99.0 * pow(256,2);
...
int update_gps(int j) {
  ...
  if (j >= gps_avg_per-1){
    ...
    if (j >=1) { // we have at least two datapoints
      ...
      Serial.print(", No. Points: "); Serial.println(speeds_size);

      add_buffer_dyn(avg_speed, tot_dist, 12);
    }
...
void setup(){
  ...
  // Report settings
  Serial.print("Measurement freqs: acc, T, gps, gps_avg: ");
  ...
  // Initialize TTN data measurements
  // data.has_location = true;
  // Serial.print(F("Your device EUI is: "));
  // Serial.println(ttn.deviceEUI());
  Serial.println("--- TTN STATUS ---");
  ttn.showStatus();
  ttn.provision(appEui, appKey);
  // is_connected = ttn.join(5); // by default infinity tries
  is_connected = false;
}
/***************************** MAIN ***************************************/
void loop(){
